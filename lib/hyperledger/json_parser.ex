defmodule Hyperledger.Parsers.JSON do
  @moduledoc """
  Parses JSON request body. Copy of Plug.Parsers.JSON but saves raw body.
  ...
  defp decode({:ok, body, conn}, decoder) do
    case decoder.decode!(body) do
      terms when is_map(terms) ->
        {:ok, terms, put_private(conn, :raw_json_body, body)}
      terms ->
        {:ok, %{"_json" => terms}, put_private(conn, :raw_json_body, body)}
    end
  rescue
